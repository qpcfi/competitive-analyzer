    ...
    <div>
      ...
      <div className="swot-grid">
        ...
        <Card title={<span style={{ color: '#1677ff', fontWeight: 600 }}>S - 优势 (Strengths)</span>} style={{ height: '100%' }}>
          <SwotItem text="GPT-4o: 编码能力领先行业基准5%" source />
          <SwotItem text="DeepSeek-V3: 1M上下文，行业首创" source />
        </Card>
        
        {/* Weaknesses */}
        <Card title={<span style={{ color: '#faad14', fontWeight: 600 }}>W - 劣势 (Weaknesses)</span>} style={{ height: '100%' }}>
          <SwotItem text="Gemini 1.5: 上下文仅32K明显落后于竞争对手 (GPT-4o 128K, Claude 200K)" source />
          <div style={{ background: '#f5f7fa', padding: '12px', borderRadius: '6px', marginBottom: '12px', borderLeft: '3px solid #faad14' }}>
            <Paragraph style={{ margin: 0 }}>所有竞品均缺乏企业级SLA承诺</Paragraph>
        ...
        <Card title={<span style={{ color: '#ff4d4f', fontWeight: 600 }}>T - 威胁 (Threats)</span>} style={{ height: '100%' }}>
          <SwotItem text="开源模型Llama 3快速迭代，成本优势持续扩大" source />
          <SwotItem text="国内大模型价格战，免费模式冲击定价体系" source />
        </Card>
      </div>

      <Alert 
        title="Critic Agent 质检意见：⚠️ 发现2处结论缺乏数据支撑，建议补充来源。" 
        type="warning" 
        showIcon 
        style={{ marginTop: 24 }}
        action={<Button size="small" type="primary" onClick={() => onOpenDrawer('re-run')}>一键修复</Button>}
      />
    </div>
  );
}
