...
engine = create_async_engine(DATABASE_URL, echo=False, pool_size=20, max_overflow=10, pool_pre_ping=True, pool_timeout=30, connect_args={"command_timeout": 60})
...
class TaskRecord(Base):
    ...
    current_collection_run_id = Column(String, nullable=True, index=True)
    current_material_ids = Column(JSON, default=list)
    ...
    task_intent = Column(JSON, default={})
...
class SourceMaterialRecord(Base):
    ...
    id = Column(String, primary_key=True)
    task_id = Column(String, ForeignKey("tasks.id"), index=True)
    collection_run_id = Column(String, nullable=True, index=True)
...
async def init_db():
    async with engine.begin() as conn:
        ...
        if engine.dialect.name == "postgresql":
            ...
            await conn.execute(text("ALTER TABLE tasks ADD COLUMN IF NOT EXISTS analysis_goal TEXT"))
            await conn.execute(text("ALTER TABLE tasks ADD COLUMN IF NOT EXISTS task_intent JSON DEFAULT '{}'::json"))
            ...
            await conn.execute(text("ALTER TABLE tasks ADD COLUMN IF NOT EXISTS active_run_id VARCHAR"))
            await conn.execute(text("ALTER TABLE tasks ADD COLUMN IF NOT EXISTS current_collection_run_id VARCHAR"))
            await conn.execute(text("ALTER TABLE tasks ADD COLUMN IF NOT EXISTS current_material_ids JSON DEFAULT '[]'::json"))
            await conn.execute(text("ALTER TABLE task_events ADD COLUMN IF NOT EXISTS run_id VARCHAR"))
            await conn.execute(text("ALTER TABLE source_materials ADD COLUMN IF NOT EXISTS collection_run_id VARCHAR"))
